...
class ConvNormAct(nn.Module):
    def __init__(self,
        ...
        self.norm_layer = nn.BatchNorm2d(
            out_channels) if norm_layer is None else norm_layer(out_channels)
...
class MultiHeadSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        q, k, v = map(lambda t: rearrange(
            t, 'b p n (h d) -> b p h n d', h=self.num_heads), qkv)
...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, emb_channels, stride=1, expand_ratio=2, act_layer=nn.ReLU):
        ...
        layers = []
        if expand_ratio != 1:
            layers.append(ConvNormAct(in_channels, hidden_dim,
...
class MobileVitBlock(nn.Module):
    def __init__(self, in_channels, out_channels, d_model, layers, mlp_dim):
        ...
        self.fusion_block2 = nn.Conv2d(
            in_channels * 2, out_channels, 3, padding=1)
    ...
    def forward(self, x):
        ...
        global_repr = rearrange(
            local_repr, 'b d (h ph) (w pw) -> b (ph pw) (h w) d', ph=2, pw=2)
        global_repr = self.transformer(global_repr)
        global_repr = rearrange(
            global_repr, 'b (ph pw) (h w) d -> b d (h ph) (w pw)', h=h//2, w=w//2, ph=2, pw=2)
